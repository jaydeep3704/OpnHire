...
import { Select, SelectTrigger, SelectContent, SelectValue, SelectGroup, SelectLabel, SelectItem, SelectSeparator } from "../ui/select";
    ...
    async function onSubmit(data: z.infer<typeof jobSchema>) {
        ...
        try {
            ...
            if(error instanceof Error && error.message!=="NEXT_REDIRECT"){
                throw new Error("An Error occured while creating job post")
            }
        }
        finally{
            setPending(false)
        ...
        <Form {...form}>
            <form className=" col-span-1 lg:col-span-2 flex flex-col gap-8" onSubmit={form.handleSubmit(onSubmit)}>
                ...
                <Button type="submit" className="w-full mt-4 flex justify-center items-center">
                    {pending ? <Loader2 className="animate-spin"/>:"Post Job"}
                </Button>
            </form>
        </Form>
    )
